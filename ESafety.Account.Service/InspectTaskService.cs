    ...
    public class InspectTaskService : FlowBusinessService, IInspectTask
    {
               ...
               && q.TaskType == (int)PublicEnum.EE_InspectTaskType.Cycle);
                ...
                var re = from t in retemp
                         ...
                         select new InspectTaskView
                         {
                             ...
                             State = t.State,
                             StateName = (PublicEnum.BillFlowState)t.State,
        ...
        public ActionResult<InspectTaskModelView> GetModel(Guid id)
        {
            try
            {
                ...
                remodel.StateName = (PublicEnum.BillFlowState)remodel.State;
        ...
        public ActionResult<IEnumerable<CSub>> GetTempTaskDangerSelector(TempTaskDangerSelect select)
        {
            try
            {
                ...
                var subs = _work.Repository<Basic_DangerPointRelation>().Queryable(p => p.DangerPointID == select.DangerPointID);
                                 ...
                                 Subs = from sub in subs
                                        ...
                                        select new Sub
                                        {
                                            ...
                                            Dangers = from d in drs
                                                      ...
                                                      where d.SubjectID == sub.SubjectID
                                                      select new Danger
                                                      {
                                                          DangerID = d.DangerID,
                                                          DangerName = dg.Name
            ...
            {
                return new ActionResult<IEnumerable<CSub>>(ex);
            }
